...
const fetcher = {
  ...
  postForm(url, formValues, customHeaders = undefined, credentials = 'include') {
    const headers = customHeaders ? new Headers(customHeaders) : new Headers();
    headers.append('Accept', 'application/json');
    headers.append('Content-Type', 'application/x-www-form-urlencoded;charset=utf-8');
    const reqOpts = { method: 'POST', credentials, headers, body: qs.stringify(formValues) };
  ...
  putJSON(url, json = {}, customHeaders = undefined, credentials = 'include') {
    const headers = customHeaders ? new Headers(customHeaders) : new Headers();
    headers.append('Accept', 'application/json');
    headers.append('Content-Type', 'application/json;charset=utf-8');
    const reqOpts = { method: 'PUT', credentials, headers, body: JSON.stringify(json) };
    const req = new Request(url, reqOpts);
    return fetch(req).then(resHandlerForJSON).then(defaultJSONHandler);
  },

  putPatchJSON(url, json = {}, customHeaders = undefined, credentials = 'include') {
    const headers = customHeaders ? new Headers(customHeaders) : new Headers();
    headers.append('Accept', 'application/json');
  ...
  del(url, params, customHeaders = undefined, credentials = 'include') {
    const headers = customHeaders ? new Headers(customHeaders) : new Headers();
    headers.append('Accept', 'application/json');
    const reqOpts = { method: 'DELETE', credentials, headers };
    const req = new Request(params ? `${url}?${qs.stringify(params)}` : url, reqOpts);
    return fetch(req).then(resHandlerForJSON).then(defaultJSONHandler);
  }
};
